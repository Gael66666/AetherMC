...
Application::Application(int &argc, char **argv) : QApplication(argc, argv)
{
    ...
    else
    {
        ...
        QDir bar(FS::PathCombine(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation), "aethermc"));
        if (bar.exists()) {
    ...
    /*
     * Establish the mechanism for communication with an already running AetherMC that uses the same data path.
...

    {
        ...
        if (!BuildConfig.FLAME_API_KEY_API_URL.isEmpty() && shouldFetch && !(capabilities() & Capability::SupportsFlame))
        {
            auto response = QMessageBox::question(nullptr,
                                                  ...
                                                  tr("Should AetherMC try to fetch the Official Curseforge Launcher's API Key? "
                                                     "Using this key technically breaks Curseforge's Terms of Service, but this distribution of AetherMC "
...
{
    if(name == "logo") {
        return QIcon(":/org.aethermc.AetherMC.svg");
    }
